...
def login_motorista(request):
    if request.method == 'POST':
        ...
        user = cadastro_motorista_collection.find_one({"placa": placa, "cpf": cpf})  # Busca pela placa e CPF

        if user:  # Verifica se o usuário foi encontrado
            return JsonResponse({'status': 'sucesso', 'dados': {}}, status=200)
        else:
            return JsonResponse({'status': 'falha', 'mensagem': 'Credenciais inválidas'}, status=401)
