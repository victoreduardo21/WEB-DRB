...
def import_planilha_to_db():
    ...
    result = sheet.values().get(spreadsheetId=SPREADSHEET_ID_CADASTRO, range=RANGE_CADASTRO).execute()
...
def login_view(request):
    if request.method == 'POST':
        ...
        user = cadastro_funcionario_collection.find_one({"email": email})  # Busca pelo email

        if user and user['senha'] == senha:  # Verifica se o email e senha estão corretos
            # Armazenar informações do usuário na sessão
            request.session['user_id'] = str(user['_id'])       # Salva o ID do usuário na sessão
            request.session['user_nome'] = user['nome']         # Salva o nome do usuário na sessão
            request.session['user_setor'] = user['setor']       # Salva o setor do usuário na sessão (ex.: Operação, Administração, Financeiro)

            messages.success(request, "Login bem-sucedido!")  # Mostra mensagem de sucesso
...
def verifica_setor(view_func):
    def wrapper(request, *args, **kwargs):
        user_setor = request.session.get('user_setor')  # Obtém o setor do usuário da sessão
